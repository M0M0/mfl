...
//  - check if nullptr or NULL is better in the context of window API!
//    ( currently using nullptr )
#pragma once
#ifndef _WIN32
...
class Window {
 ...
 public:
  Window(HINSTANCE hInstance) : _hWnd(nullptr),_hInstance(hInstance),
				WC(hInstance),_ExitCode(-1),_IsExit(false) {
  ...
  virtual void Create(HWND hWndParent, DWORD dwStyles, int width = 300,
		      int height = 300) {
    _hWnd = CreateWindowEx(WS_EX_CLIENTEDGE,CLASS_NAME,CLASS_TITLE,dwStyles,0,0,
			   width,height,hWndParent,NULL,_hInstance,(void*)this);
    DWORD err = ::GetLastError();
    if (err > 0 || !_hWnd)
      THROW_RT_ERR("Window Creation failed with errorcode \'"
		   +std::to_string(static_cast<_ULonglong>(err))+"\'!");
  }
  ...
  const bool handleMessages() {
	...
	THROW_RT_ERR("Window is marked for exit\
                      but quitmessage was not returned!");
 ...
 protected:
  ...
  struct WndClass { //Internal window class
    WNDCLASSEX		_wc;
    WndClass(HINSTANCE hInstance){
      ...
      if (!RegisterClassEx(&_wc)) THROW_RT_ERR("Error registering Class");
    }
    ~WndClass() { 
 ...
 private:
  ...
  MessageIterator GetMessageHandler(long Msg) {
    auto it = _MsgHandlers.find(Msg);
    return it;
  }
  const MessageIterator	NullMessageIterator() { return _MsgHandlers.end(); }

  static LRESULT CALLBACK MsgRouter(HWND hWnd, UINT uMsg, WPARAM wParam, 
				    LPARAM lParam) {
    Window* wnd = nullptr;
    if (uMsg==WM_NCCREATE){
