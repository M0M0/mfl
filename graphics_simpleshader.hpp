...
public:
    ...
    void setAttrib(char const * name) {
        ...
        glBindAttribLocation(program,num_attribs_++,name);
    }
    GLuint getAttrib(char const * name) const {
        return attrib_[name];
    }
    void resetAttribs() {
        attrib_.clear();
    ...
    //==========================================================================
    void addShader_VS(char const * shader_source) {
        _createShaderFromSource(shader_source,GL_VERTEX_SHADER,vertex_shader_);
    }
    void addShader_GS(char const * shader_source) {
        _createShaderFromSource(shader_source,GL_GEOMETRY_SHADER,
                                geometry_shader_);
    } 
    void addShader_FS(char const * shader_source) {
        _createShaderFromSource(shader_source,GL_FRAGMENT_SHADER,
                                fragment_shader_);
    }
    //==========================================================================
    ...
    void link() {
        ...
        if (vertex_shader_ > 0) glAttachShader(program,vertex);
        if (geometry_shader_ > 0) glAttachShader(program,geometry);
        if (fragment_shader_ > 0) glAttachShader(program,fragment);
...
private:
    ...
    void _createShaderFromSource(char const * source,GLenum shader_type,
                                 GLuint shader_id) {
        if (shader_id) _deleteShader(shader_id); // throw exception
    ...
    void _shaderCompileCheck(GLuint shader_id) {
        ...
        if (success != GL_TRUE) _shaderInfoLog_THROW(shader_id);
    }
    void _shaderInfoLog_THROW(GLuint shader_id) {
    ...
    GLint _shader_glGet(GLuint shader_id,GLenum what) {
        GLint param;
    ...
    void _programLinkCheck() {
        ...
        if (success != GL_TRUE) _programInfoLog_THROW();
    }
    ...
    GLint _program_glGet(GLuint program_id,GLenum what) {
        GLint param;
