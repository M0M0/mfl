...
namespace gl{
  void ShaderCompileCheck(GLint shader) {
    if (GLGET(Shader,shader,GL_COMPILE_STATUS) == GL_TRUE) return;
  ...
  void ProgramLinkCheck(GLint program){
    if (GLGET(Program,program,GL_LINK_STATUS) == GL_TRUE) return;
    THROW_RT_ERR(
  ...
  // simple glsl compiler
  // only for basic setups of vertex & fragment shaders
  // with glsl shaders using location
  GLuint GetProgram_VertexFragmentShader(char const * vertex_source,
