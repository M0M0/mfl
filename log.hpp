...
namespace MFL{
	...
	public:
		...
		virtual ~Log(){
			std::ofstream file(_filename.c_str(),std::ios_base::out|std::ios_base::trunc);
	...
	protected:
		std::string		_data;
		// formating operator
		virtual std::string logformat() = 0;
	};
...
#pragma warning(disable:4996)
	...
	public:
		...
		void Error  (const std::string& str){
			_data += logformat() + "<-Err-> " + strformat(str) + '\n';
		} 
		...
		void Message(const std::string& str){
			_data+= logformat() + "<-Msg-> " + strformat(str) + '\n';
		}
