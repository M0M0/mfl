...
public:
	explicit GLWindow(HINSTANCE hInstance,GLInterface*&& ptr)
		:Window(hInstance),_GLptr(ptr),_hDC(nullptr),_hGLRC(nullptr),_mouse()
	{
	...
	void Create(HWND hWndParent,DWORD dwStyles,int width=600,int height=600)
	{
		...
		if(WGLEW_ARB_pixel_format){
			int pixAttribs[] = {	WGL_SUPPORT_OPENGL_ARB, TRUE,
									...
									0};
			UINT pixCount = 0;
			if(!wglChoosePixelFormatARB(_hDC,pixAttribs,nullptr,1,&pixelformat,&pixCount) || pixelformat == -1)
		...
		if(WGLEW_ARB_create_context){
			...
			if(GLEW_VERSION_4_0){
				...
				THROW_RT_ERR("There was an unexpected error using WGLEW_ARB_create_context!");

#ifdef USING_ERRORLOG	
			::ERRORLOG_NAME.Message("Creating GL %i.%i Context.",attribs[1],attribs[3]);
#endif
	...
	static int GetGLError()
	{
		...
		case GL_INVALID_VALUE:
			str = "GL Invalid value.";
			break;
		case GL_INVALID_OPERATION:
			str = "GL Invalid Operation.";
			break;
		case GL_STACK_OVERFLOW:
			str = "GL Stack Overflow.";
			break;
		case GL_STACK_UNDERFLOW:
			str = "GL Stack Underflow.";
			break;
		case GL_OUT_OF_MEMORY:
			str = "GL Out of Memory.";
			break;
...
private:
	...
	// Mousebutton pressed
	//=====================================================================================
	static long OnMouseLPressed (Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
	...
	static long OnMouseRPressed (Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
	...
	static long OnMouseMPressed (Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
	...
	//=====================================================================================
	static long OnMouseLReleased(Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
	...
	static long OnMouseRReleased(Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
	...
	static long OnMouseMReleased(Window& wnd,HWND hWnd,WPARAM wParam,LPARAM lParam){
		GLWindow& glwnd = reinterpret_cast<GLWindow&>(wnd);
