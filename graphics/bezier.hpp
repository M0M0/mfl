...
class BezierSurface{
 ...
 public:
  ...
  /// \param[in] last   Iterator after the last point.
  template <typename Iterator>
  BezierSurface(Iterator first,Iterator last) : controlpoints_(first,last) {
    //static_assert(std::is_same<
    //  std::iterator_traits<Iterator>,std::vector<glm::vec3>>::value,
    //  "Value type of iterator is not \'glm::vec3\'.");
  }

  BezierCurve& GetArray(uint index) {
    if (controlpoints_.size() <= index) controlpoints_.resize(index + 1);
    return *reinterpret_cast<BezierCurve*>(&controlpoints_[index]);
  }
...
private:
  ...
  Points controlpoints_; ///< The controlpoints_ of the bezier surface.
};
